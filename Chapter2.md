...
6. 一个三角形是否覆盖某个像素，这取决于你如何设置GPU 的管线，例如，你也许是用**点取样（point sampling）**来决定是否“在里面”。最简单的方案是使用没有像素的中心点，因此如果这个中心点在三角形内，那么就相当于整个像素也在三角形内了。你也可以对每个像素使用超过一个的样本，例如使用**超级采样（supersampling）** 或者**多重采样抗锯齿技术（multisampling antialiasing techniques）**，你也可以使用**保守光栅化技术（conservative rasterization）**，他定义是一个像素有一点点覆盖到了三角形则认为该像素在三角形内。
