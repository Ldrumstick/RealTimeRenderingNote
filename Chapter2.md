...
9. 这三个阶段是按一下顺序执行的：**曲面细分** -> **几何着色器** -> **流输出** ，他们都可选择的。不管他们是否被选，管线接下来都会检查这组齐次坐标顶点是否在摄像机视野中22222221
...
## 屏幕映射 Screen Mapping
1. 只有被裁剪过的在视体中的图元才会被传递到屏幕映射阶段。并且坐标仍然是三维坐标当进入这个阶段时。
2. 每个图元x轴坐标与y轴坐标被变换为**屏幕坐标（Screen Coordinate）** 
3. 屏幕坐标与z轴坐标一起也可以被称作为**窗口坐标（Window Coordinate）**
4. 假定场景应该被渲染在一个最小角在（x1,y1）且最大角在（x2,y2）且x1 < x2,y1 < y2的窗口中
5. 屏幕映射是一个缩放操作变换，新的想x,y坐标被称为屏幕坐标
6. z坐标(在OpenGL中为[-1,+1],DirectX中为[0,1])也会被映射到[z1,z2]中，且默认值为z1=0 ,z2=1，当然他们可以同个API进行更改。
7. 连同冲映射后的z值得窗口坐标会被传递到光栅化阶段
![Aaron Swartz](Image/2_4.jpg)

8. 提供一个水平的像素数组，并且使用笛卡尔坐标系，最左边的像素的最边缘的浮点坐标是0.0。OpenGL一直使用这种设定，DirectX10也继承了这种设定。该像素的正中心是0.5。因此，一组[0,9]的像素覆盖的范围是[0.0,10.0]。公式如下：
~~~
d=floor(c)
c=d+0.5
~~~

*d* 表示像素的离散下标（整数）。*c* 这个像素内的连续值（浮点数）

9. 所有的API的像素位置值都是从左边向右边递增的。零坐标是在上面还是下面在OpenGL和DirectX中是不一样的。OpenGL自始至终喜欢笛卡尔坐标系，认为左下角是最小值元素(0,0)。然而DirectX有时候定义左上角是最小值元素(0,0)，这取决于上下文。例如，在OpenGL中一个图片的(0,0)点在左下角，而DirectX在左上角。这对于从这个API移植到另一个是非常重要的

## 光栅化 Rasterization
1. 有了被变换和投影过的顶点以及他们相关连的着色数据（所有都来自几何阶段），下一阶段的目标是找到所有需要被渲染的图元的对应像素。可以简称为**图像元素（picture elements）**。
2. 一个三角形正在被渲染，我们称这个过程为**光栅化（rasterization）**，他被分成两个方法子阶段：**三角形设置（triangle setup）**，也叫做**图元装配（primitive assembly）** ，和 **三角形遍历（triangle traversal）** 。
3. 需要注意的是他们也可以处理点和线，虽然因为经常被用来处理三角形，所以名称中带有“三角形”
4. **光栅化**也被叫做**扫描转换（scan conversion）** ，是因为扫描屏幕中的二维顶点也包括每个顶点的z值（深度值）以及各种各样顶点相关的着色信息转换为屏幕上的像素。
![Aaron Swartz](Image/2_5.jpg)
5. 光栅化也可以认为是几何阶段和像素阶段的一个同步点，因为在这阶段，三个顶点被形成为一个三角形，并最终发送到像素阶段
6. 一个三角形是否覆盖某个像素，这取决于你如何设置GPU 的管线，例如，你也许是用**点取样（point sampling）** 来决定是否“在里面”。最简单的方案是使用每个像素的中心点，因此如果这个中心点在三角形内，那么就相当于整个像素也在三角形内了。你也可以对每个像素使用超过一个的样本，例如使用**超级采样（supersampling）** 或者**多重采样抗锯齿技术（multisampling antialiasing techniques）** ，你也可以使用**保守光栅化技术（conservative rasterization）**，他定义是一个像素有一点点覆盖到了三角形则认为该像素在三角形内。

### 三角形设置 Triangle Setup
1. 在这个阶段，三角形的微分，边缘方程，和其他数据被计算，这些数据也许被用于**三角形遍历**，也可能用于对几何阶段产生个各种着色数据进行插值。这个任务有硬件的固定方法处理（由硬件支持）

### 三角形遍历Triangle Traversal
1. 在这个阶段，每个像素都有一个中心点或者采样点用来检查是否被三角形覆盖，这个三角形覆盖的像素会产生一个**片元（fragment）** 。
2. 寻找哪些中心点或者采样点在三角形中的过程经常被叫做**三角形遍历（Triangle Traversal）** 
3. 每个三角形片元的属性通过使用三角形顶点的数据进行插值产生，这些属性包括片元的深度，以及任何来自几何阶段的着色数据。三角形的**透视矫正插值（perspective-correct interpolation）** 也会被执行。所有在图元中的像素或者采样点在随后都会被发送给**像素处理阶段（pixel processing stage）**

## 像素处理 Pixel Processing
1. 到此步，所有三角形或其他图元覆盖的像素通过先前一系列的步骤都被找到了。
2. **像素处理阶段** 被分为**像素着色（pixel shading）** 和**合并（merging）** 
3. 像素处理是对在一个图元中的像素或者图元进行逐像素或者逐采样点计算和操作的阶段

### 像素着色 Pixel Shading
1.任何的逐像素着色计算都会在这个阶段被执行，且使用插值后的着色数据作为输入，最后结果是会被发送给下一阶段的一个或多个颜色
2.不像三角形的设置和遍历阶段，他们通常是在专门的硬件中计算的，而像素着色阶段是被可编程GPU核心执行的。
3.为此，程序员为**像素着色器(pixel shader)** 或者叫做在OpenGL中更为熟知的**片元着色器（fragment shader）** ，它可以控制任何想要的计算
4.大量各式的技术可以在这被使用。其中最重要的一个就是**纹理（texture）** ，简单来说，texturing an object 的意思是为了各种各样的目的，“粘合”一个或者多个图像在一个物体上
5.图像也许是一维、二维、或者三维的，其中二维的是最为常见的
6.最简单的理解，每个片元最后都会产生一个颜色值然后发送给下一个阶段

### 合并 Merging
1. 每个像素的信息都被存储在**颜色缓冲color buffer** 中，**颜色缓冲color buffer** 是一个颜色的矩形数组（每个颜色包含红、绿、蓝三个部分）
2. **合并 Merging**阶段的职责是将由**像素着色阶段**产生的片元颜色与当前存储在**color buffer** 中的颜色进行结合。因此这个阶段也叫作**ROP**，意思是**raster operations （pipeline）**或者**render output unit** 
3. 不像着色阶段，GPU单元执行这个阶段不是完全可编程的，然而是高度可配置的，能够实现大量的效果
4. 这个阶段也负责分析可见性。意味着，当某个场景被渲染，**颜色缓冲** 应该包含场景中摄像机能够看到的所有图元的颜色。
5. 对于大多数甚至全部的图形硬件来说，都是通过**z轴缓冲（z-buffer）**（也被称作**深度缓冲（depth buffer）** ）算法来完成的。
6. **z-buffer** 与**color buffer** 有着相同的大小和形状，每个像素存储的是当前最近的图元的z值。这意味着当一个图元被渲染到一个确定的像素是，这个图元在这个像素上的z值被计算并与**z-buffer** 中相同像素位置的内容相比较。如果新的z值比 **z-buffer** 中的z值小，那么这个图元在这个像素上离摄像机更近被渲染，相比于这上一个离摄像机最近的图元。因此，这个在这个像素上的z值，和颜色值会被更新成这个图元的z值和颜色值用于被绘制。如果计算的z值比**z-buffer** 中的值大，那么**z-buffer** 和 **color-buffer** 会原封不动。
7. z-buffer算法是简单的，拥有O（n）收敛（n表示被渲染的图元数），可以在任何可以计算相关联像素z值的绘制的图元上工作。
8. 但是要注意的是，这个算法允许大多数图元在任何指令下绘制，这是它受欢迎的另一个原因。
9. 然而，**z-buffer** 存储的只是每个屏幕上的点的单个深度值，因此它不可以被用在部分透明的图元上。必须要渲染完所有的不透明吐艳，然后在从后往前指令，或者使用一个单独的顺序无关算法。透明度是基础**z-buffer** 重要的缺点之一
10. **颜色缓冲区（color buffer）** 被用来存储每个像素的颜色，**深度缓冲区（z-buffer）** 被用来存储每个像素的z值。然而，还有其他的通道（channel）和缓冲区（buffer）可以被使用来过滤和捕获片元信息。
11. **阿尔法通道（alpha channel）**与color buffer相关联，每个像素存储一个相关不透明度值。在旧的API中，alpha通道被用于有选择剔除像素，通过alpha测试特性。现在一个剔除操作可以放入到像素着色器程序中，任何可被计算的类型都可以作为一个剔除的触发器。这个类型的测试可以被用于确保完全透明的片元不会对z-buffer产生影响
12. **模板缓冲区（stencil buffer）** 是一个屏幕之外的缓冲区，被用于记录被渲染图元的的位置。典型的它每像素包含8bit。图元可以通过各种各样的方法被渲染在模板缓冲区，这个缓冲区的内容随后被用于控制渲染进颜色缓冲区和深度缓冲区。举个例子，设想一个实心圆已经被渲染进入了**模板缓冲区** 。这里可以关联一个操作：只允许随后位于圆内的图元进入颜色缓冲区。模板缓冲区对于实现一些特殊的效果是一个强力的工具。所有位于这个管线末端的方法都被称为**栅格操作（raster operations （ROP））** 或者称为 **混合操作（blend operations）** 。它使得 对当前颜色缓冲中的颜色与正在被处理的三角形内的像素颜色进行混合成为可能。它也可以影响例如半透明度或者颜色样本的混合方式。如上所述。混合（blending）是典型的通过API可配置的，不可以完全可编程的。然而，有些API已经提供了拥有允许可编程混合能力的栅格指令界面，也被称为像素着色器指令。
13. **帧缓冲区（framebuffer）** 通常是由系统中所有缓冲区组成
14. 当图元到达并通过了光栅化阶段，那些在摄像机视野范围内可见的被显示在屏幕中。屏幕中显示的内容是颜色缓冲区。为了避免人们看见正在被光栅化和发送给屏幕的图元，**双缓冲区（double buffer）**被使用。这意味着，被渲染的场景放在屏幕之外，叫做**后置缓冲区 （back buffer）** 。当场景被渲染在后置缓冲区中后，后置缓冲区中的内容就之前在屏幕上显示的**前置缓冲区（front buffer）** 的内容进行交换。这种交换经常发生在**垂直回扫（vertical retrace）**期间，确保交换是不影响显示的时间点。